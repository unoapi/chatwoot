...
class Webhooks::NotificaMeEventsJob < ApplicationJob
  ...
  def process_event_params(channel, params)
    if params['type'] == 'MESSAGE_STATUS'
      ...
      source_id = %w[SENT REJECTED ERROR].include?(messageStatus) ? messageId : messageProviderId
      ...
      if %w[ERROR REJECTED].include?(messageStatus)
      ...
      ActiveRecord::Base.transaction do # rubocop:disable Metrics/BlockLength
        ms = message['contents'].map do |c| # rubocop:disable Metrics/BlockLength
          ...
          if c['type'] != 'text'
            attachment_file = nil
            if ['whatsapp_business_account'].include?(channel.notifica_me_type) # rubocop:disable Performance/CollectionLiteralInLoop
              ...
              tempfile = Tempfile.new('notificamehub', binmode: true)
  ...
  def find_or_create_conversation(contact_inbox)
    conversation = if contact_inbox.inbox.lock_to_single_conversation
